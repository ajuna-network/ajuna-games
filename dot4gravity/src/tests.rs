...
fn a_player_drops_a_bomb() {
    ...
    let drop_bomb_result = Game::drop_bomb(&mut game_state, bomb_position.clone(), player);
...
fn a_cell_can_hold_one_or_more_bombs_from_different_players() {
    ...
    let drop_bomb_result = Game::drop_bomb(&mut game_state, bomb_position.clone(), player1);
    assert!(drop_bomb_result.is_ok());
    ...
    let drop_bomb_result = Game::drop_bomb(&mut game_state, bomb_position.clone(), player2);
    assert!(drop_bomb_result.is_ok());
...
fn a_player_cannot_place_more_than_one_bomb_in_a_cell() {
    ...
    // Drop the first bomb
    let dropping_bomb_result = Game::drop_bomb(&mut game_state, position, player);
    assert!(
...
fn a_stone_dropped_from_north_side_should_move_until_it_reaches_an_obstacle() {
    ...
    let state = Game::drop_stone(state, player1, Side::North, 0).unwrap();
    assert_eq!(
        state.board.get_cell(&Coordinates { row: 9, col: 0 }),
        Cell::Stone(player1)
    );
    let state = Game::drop_stone(state, player2, Side::North, 1).unwrap();
    assert_eq!(
        state.board.get_cell(&Coordinates { row: 8, col: 1 }),
        Cell::Stone(player2)
    );
    let state = Game::drop_stone(state, player1, Side::North, 2).unwrap();
    assert_eq!(
        state.board.get_cell(&Coordinates { row: 0, col: 2 }),
        Cell::Stone(player1)
    );
    assert_eq!(
        Game::drop_stone(state, player2, Side::North, 3).unwrap_err(),
...
fn a_stone_dropped_from_south_side_should_move_until_it_reaches_an_obstacle() {
    ...
    let state = Game::drop_stone(state, player1, Side::South, 0).unwrap();
    assert_eq!(
        state.board.get_cell(&Coordinates { row: 0, col: 0 }),
        Cell::Stone(player1)
    );
    let state = Game::drop_stone(state, player2, Side::South, 1).unwrap();
    assert_eq!(
        state.board.get_cell(&Coordinates { row: 1, col: 1 }),
        Cell::Stone(player2)
    );
    let state = Game::drop_stone(state, player1, Side::South, 2).unwrap();
    assert_eq!(
        state.board.get_cell(&Coordinates { row: 9, col: 2 }),
        Cell::Stone(player1)
    );
    assert_eq!(
        Game::drop_stone(state, player2, Side::South, 3).unwrap_err(),
        GameError::InvalidDroppingPosition
    );
}
...
fn a_stone_dropped_from_east_side_should_move_until_it_reaches_an_obstacle() {
    ...
    let state = Game::drop_stone(state, player1, Side::East, 0).unwrap();
    assert_eq!(
        state.board.get_cell(&Coordinates { row: 0, col: 0 }),
        Cell::Stone(player1)
    );
    let state = Game::drop_stone(state, player2, Side::East, 1).unwrap();
    assert_eq!(
        state.board.get_cell(&Coordinates { row: 1, col: 1 }),
        Cell::Stone(player2)
    );
    let state = Game::drop_stone(state, player1, Side::East, 2).unwrap();
    assert_eq!(
        state.board.get_cell(&Coordinates { row: 2, col: 9 }),
        Cell::Stone(player1)
    );
    assert_eq!(
        Game::drop_stone(state, player2, Side::East, 3).unwrap_err(),
        GameError::InvalidDroppingPosition
    );
}
#[test]
fn a_stone_dropped_from_west_side_should_move_until_it_reaches_an_obstacle() {
    ...
    let state = Game::drop_stone(state, player1, Side::West, 0).unwrap();
    assert_eq!(
        state.board.get_cell(&Coordinates { row: 0, col: 9 }),
        Cell::Stone(player1)
    );
    let state = Game::drop_stone(state, player2, Side::West, 1).unwrap();
    assert_eq!(
        state.board.get_cell(&Coordinates { row: 1, col: 8 }),
        Cell::Stone(player2)
    );
    let state = Game::drop_stone(state, player1, Side::West, 2).unwrap();
    assert_eq!(
        state.board.get_cell(&Coordinates { row: 2, col: 0 }),
        Cell::Stone(player1)
    );
    assert_eq!(
        Game::drop_stone(state, player2, Side::West, 3).unwrap_err(),
        GameError::InvalidDroppingPosition
    );
}
...
fn a_stone_should_explode_a_bomb_when_passing_through() {
    ...
    let dropping_stone_result = Game::drop_stone(state.clone(), 0, Side::North, 5);
    ...
    let dropping_stone_result = Game::drop_stone(state.clone(), 1, Side::North, 8);
    assert!(dropping_stone_result.is_ok());
    state = dropping_stone_result.unwrap();
    assert_eq!(
    ...
    let dropping_stone_result = Game::drop_stone(state.clone(), 0, Side::East, 7);
    assert!(dropping_stone_result.is_ok());
    state = dropping_stone_result.unwrap();
    assert_eq!(
    ...
    let dropping_stone_result = Game::drop_stone(state.clone(), 1, Side::South, 4);
    assert!(dropping_stone_result.is_ok());
    state = dropping_stone_result.unwrap();
    assert_eq!(
    ...
    let dropping_stone_result = Game::drop_stone(state.clone(), 0, Side::South, 4);
    assert!(dropping_stone_result.is_ok());
    state = dropping_stone_result.unwrap();
    assert_eq!(
        state.board.get_cell(&Coordinates { row: 4, col: 4 }),
...
fn should_play_a_game() {
    ...
    assert!(result.is_ok());
    assert_eq!(state.bombs[player1 as usize], player1_num_bombs - 1);

    let dropping_result = Game::drop_bomb(&mut state, Coordinates { row: 0, col: 0 }, player1);
    assert!(
    ...
    let result = Game::drop_bomb(&mut state, Coordinates { row: 9, col: 9 }, player1);
    assert!(result.is_ok());
    assert_eq!(state.bombs[player1 as usize], player1_num_bombs - 2);

    let result = Game::drop_bomb(&mut state, Coordinates { row: 7, col: 7 }, player1);
    assert!(result.is_ok());
    assert_eq!(state.bombs[player1 as usize], player1_num_bombs - 3);

    let result = Game::drop_bomb(&mut state, Coordinates { row: 6, col: 8 }, player1);
    assert!(result.is_err());
    assert_eq!(
        result.unwrap_err(),
    ...
    assert!(result.is_ok());
    assert_eq!(state.bombs[player2 as usize], player2_num_bombs - 1);

    let result = Game::drop_bomb(&mut state, Coordinates { row: 9, col: 9 }, player2);
    assert!(
        result.is_ok(),
        "A cell should hold bombs of different players"
    );
    assert_eq!(state.bombs[player2 as usize], player2_num_bombs - 2);

    let result = Game::drop_bomb(&mut state, Coordinates { row: 9, col: 3 }, player2);
    assert!(result.is_ok());
    assert_eq!(state.bombs[player2 as usize], player2_num_bombs - 3);
    ...
    let dropping_result = Game::drop_stone(state.clone(), player2, Side::North, 0);
    assert!(dropping_result.is_err());
    assert_eq!(dropping_result.unwrap_err(), GameError::NotPlayerTurn);

    let dropping_result = Game::drop_stone(state.clone(), player1, Side::North, 0);
    ...
    state = Game::drop_stone(state.clone(), player2, Side::North, 2).unwrap();
    state = Game::drop_stone(state.clone(), player1, Side::South, 8).unwrap();
    state = Game::drop_stone(state.clone(), player2, Side::North, 2).unwrap();
    state = Game::drop_stone(state.clone(), player1, Side::South, 8).unwrap();
    state = Game::drop_stone(state.clone(), player2, Side::North, 2).unwrap();

    // player 1 explodes bomb on 9,3 and player 2 loses stones on 9,2 and 8,2
    state = Game::drop_stone(state.clone(), player1, Side::North, 3).unwrap();
    assert_eq!(
        state.board.get_cell(&Coordinates { row: 9, col: 2 }),
        Cell::Empty
    );
    ...
    assert!(state.winner.is_some());
    assert_eq!(state.winner.unwrap(), player1);
}
